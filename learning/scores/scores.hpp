...
public:
    ...
    virtual double local_score_wbm(const BayesianNetworkBase& model, const std::shared_ptr<BandwidthSelector> m_bselector, const std::string& variable) const {
        auto parents = model.parents(variable);
        return local_score_wbm(model, m_bselector, variable, parents);
    }

    virtual double local_score(const BayesianNetworkBase& model,
                               const std::string& variable,
                               const std::vector<std::string>& parents) const = 0;
    virtual double local_score_wbm(const BayesianNetworkBase& model,
...
public:
    ...
    virtual double vlocal_score_wbm(const BayesianNetworkBase& model, const std::shared_ptr<BandwidthSelector> m_bselector, const std::string& variable) const {
        auto parents = model.parents(variable);
        return vlocal_score_wbm(model, m_bselector, variable, parents);
    }

    virtual double vlocal_score(const BayesianNetworkBase& model,
                                const std::string& variable,
                                const std::vector<std::string>& parents) const = 0;
    virtual double vlocal_score_wbm(const BayesianNetworkBase& model,
                               const std::shared_ptr<BandwidthSelector> m_bselector, 
                               const std::string& variable,
                               const std::vector<std::string>& parents) const = 0;
