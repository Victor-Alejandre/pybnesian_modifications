...
public:
    ...
    virtual std::shared_ptr<Factor> new_factor_wbm(const BayesianNetworkBase&,
                                               const std::string&,
                                               const std::vector<std::string>&,
...
template <typename F>
std::shared_ptr<F> generic_new_factor_ckde(const std::string& variable,
                                      const std::vector<std::string>& evidence,
                                      const std::shared_ptr<BandwidthSelector> m_bselector,
                                      py::args args,
                                      py::kwargs kwargs) {
    if (args.empty() && kwargs.empty())
        return std::make_shared<F>(variable, evidence, m_bselector);
    else {
        auto type = py::type::handle_of<F>();
        auto obj = type(variable, evidence, m_bselector,*args, **kwargs);
        return obj.template cast<std::shared_ptr<F>>();
    }
}
