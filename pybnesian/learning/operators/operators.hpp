...
public:
    ...
    void cache_local_scores_wbm(const BayesianNetworkBase& model, const Score& score, const std::shared_ptr<BandwidthSelector> m_bselector) {
        if (m_local_score.rows() != model.num_nodes()) {
            m_local_score = VectorXd(model.num_nodes());
        }

        for (const auto& node : model.nodes()) {
            m_local_score(model.collapsed_index(node)) = score.local_score_wbm(model, m_bselector, node);
        }
    }
    ...
    void cache_vlocal_scores_wbm(const BayesianNetworkBase& model, const ValidatedScore& score, const std::shared_ptr<BandwidthSelector> m_bselector) {
        if (m_local_score.rows() != model.num_nodes()) {
            m_local_score = VectorXd(model.num_nodes());
        }

        for (const auto& node : model.nodes()) {
            m_local_score(model.collapsed_index(node)) = score.vlocal_score_wbm(model, m_bselector, node);
        }
    }
    ...
    void update_local_score_wbm(const BayesianNetworkBase& model, const Score& score, const std::string& variable, const std::shared_ptr<BandwidthSelector> m_bselector) {
        m_local_score(model.collapsed_index(variable)) = score.local_score_wbm(model, m_bselector, variable);
    }
    ...
    void update_vlocal_score_wbm(const BayesianNetworkBase& model,
                             const ValidatedScore& score,
                             const std::string& variable,
                             const std::shared_ptr<BandwidthSelector> m_bselector) {
        m_local_score(model.collapsed_index(variable)) = score.vlocal_score_wbm(model, m_bselector, variable);
    }
...
public:
    ...
    virtual void update_scores_wbm(const BayesianNetworkBase&, const Score&, const std::vector<std::string>&, const std::shared_ptr<BandwidthSelector> m_bselector) = 0;
...
public:
    ...
    void update_scores_wbm(const BayesianNetworkBase&, const Score&, const std::vector<std::string>&,
                             const std::shared_ptr<BandwidthSelector>) override;
...
public:
    ...
    void update_scores_wbm(const BayesianNetworkBase&,
                           const Score&,
                           const std::vector<std::string>&,
                           const std::shared_ptr<BandwidthSelector> m_bselector) override;
    //hay que cambiar esto para semiparametric//
    void cache_scores(const ConditionalBayesianNetworkBase& model, const Score& score) override {
...
public:
    ...
    void update_scores_wbm(const BayesianNetworkBase& model,
                       const Score& score,
                       const std::vector<std::string>& variables,
                       const std::shared_ptr<BandwidthSelector> m_bselector) override {
        update_scores_wbm<>(model, score, variables, m_bselector);
    }
    // Hay que cambiar esto ya que es util para juntar opsets, redefinir abajo update_score<> y definir como la anterior
    void cache_scores(const ConditionalBayesianNetworkBase& model, const Score& score) override {
    ...
    template <typename M>
    void update_scores_wbm(const M& model, const Score& score, const std::vector<std::string>& variables, const std::shared_ptr<BandwidthSelector> m_bselector);
...
template <typename M>
void OperatorPool::update_scores_wbm(const M& model, const Score& score, const std::vector<std::string>& variables, const std::shared_ptr<BandwidthSelector> m_bselector) {
    raise_uninitialized();

    if (owns_local_cache()) {
        for (const auto& n : variables) {
            m_local_cache->update_local_score_wbm(model, score, n, m_bselector);
        }
    }

    for (auto& op_set : m_op_sets) {
        op_set->update_scores_wbm(model, score, variables, m_bselector);
    }
}
