...
double CVLikelihood::local_score_wbm(const BayesianNetworkBase& model,
                                const std::shared_ptr<BandwidthSelector> m_bselector,
                                const std::string& variable,
                                const std::vector<std::string>& evidence) const {
    return local_score_wbm(model, model.underlying_node_type(m_cv.data(), variable), m_bselector, variable, evidence);
}
