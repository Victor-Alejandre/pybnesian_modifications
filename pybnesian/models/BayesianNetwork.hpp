...
using factors::Factor, factors::Arguments, factors::UnknownFactorType, factors::continuous::CKDEType;
...
public:
    ...
    virtual void fit_wbm(const DataFrame& df, const std::shared_ptr<BandwidthSelector> m_bselector, const Arguments& construction_args =  Arguments()) = 0;
...
public:
    ...
    void fit_wbm(const DataFrame& df, const std::shared_ptr<BandwidthSelector> m_bselector, const Arguments& construction_args =  Arguments()) override;
...
template <typename DagType>
void BNGeneric<DagType>::fit_wbm(const DataFrame& df, const std::shared_ptr<BandwidthSelector> m_bselector, const Arguments& construction_args) {
    if (m_cpds.empty()) {
        m_cpds.resize(num_raw_nodes());
    }
    ...
    for (const auto& nn : nodes()) {
        ...
        if (*node_type_ == CKDEType::get_ref()){
        auto [args, kwargs] = construction_args.args(nn, node_type_);
        m_cpds[i] = node_type_->new_factor_wbm(*this, nn, p, m_bselector, args, kwargs);
        m_cpds[i]->fit(df);
        }
        else {
        auto [args, kwargs] = construction_args.args(nn, node_type_);
