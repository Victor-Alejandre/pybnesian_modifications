...
:returns: Local score value of ``node`` in the ``model`` with ``evidence`` as parents and ``variable_type`` as
        ...
        .def(
            "local_score_wbm",
            [](const CppClass& self, const BayesianNetworkBase& m, const std::shared_ptr<BandwidthSelector> m_bselector, const std::string& variable) {
                return self.local_score_wbm(m, m_bselector, variable);
            },
            py::arg("model"),
            py::arg("m_bselector"),
            py::arg("variable"),
            R"doc(
Returns the local score value of a node ``variable`` in the ``model`` with ``bandwidth_selector``.

For example:

.. code-block:: python

    >>> score.local_score(m, "a", UCV())

returns the local score of node ``"a"`` in the model ``m``. This method assumes that the parents in the score are
``m.parents("a")`` and its node type is ``m.node_type("a")``.

:param model: Bayesian network model.
:param m_bselector: A bandwidth selection method.
:param variable: A variable name.
:returns: Local score value of ``node`` in the ``model``.
)doc")
        .def(
            ...
            [](const CppClass& self,
               ...
               const std::string& variable,
               const std::vector<std::string>& evidence) { return self.local_score_wbm(m, m_bselector, variable, evidence); },
            py::arg("model"),
            py::arg("m_bselector"),
            py::arg("variable"),
            py::arg("evidence"),
            R"doc(
Returns the local score value of a node ``variable`` in the ``model`` if it had ``evidence`` as parents with ``m_bselector`` the bandwidth selection method.

For example:

.. code-block:: python

    >>> score.local_score(m, UCV(), "a", ["b"])
...
:returns: Local score value of ``node`` in the ``model`` with ``evidence`` as parents.
        ...
        .def(
            ...
            [](CppClass& self,
               ...
               const std::string& variable,
               const std::vector<std::string>& evidence) {
                return self.local_score_wbm(m, variable_type, m_bselector, variable, evidence);
            },
            py::arg("model"),
            py::arg("variable_type").none(false),
            py::arg("m_bselector"),
            ...
            R"doc(
Returns the local score value of a node ``variable`` in the ``model`` if its conditional distribution were a
``variable_type`` factor and it had ``evidence`` as parents with ``m_bselector`` the bandwidth selection method.

For example:

.. code-block:: python

    >>> score.local_score(m, LinearGaussianCPDType(), UCV(), "a", ["b"])
...
:returns: Validated local score value of ``node`` in the ``model`` with ``evidence`` as parents and ``variable_type`` as
        ...
        .def(
            "vlocal_score_wbm",
            [](const CppClass& self, const BayesianNetworkBase& m, const std::shared_ptr<BandwidthSelector> m_bselector, const std::string& variable) {
                return self.vlocal_score_wbm(m, m_bselector, variable);
            },
            py::arg("model"),
            py::arg("m_bselector"),
            py::arg("variable"),
            R"doc(
vlocal_score(self: pybnesian.ValidatedScore, model: BayesianNetworkBase or ConditionalBayesianNetworkBase, m_bselector: BandwidthSelector, variable: str) -> float

Returns the validated local score value of a node ``variable`` in the ``model``, with ``m_bselector`` as bandwidth selector.

For example:

.. code-block:: python

    >>> score.local_score(m, UCV(), "a")
...
:returns: Validated local score value of ``node`` in the ``model``.
        ...
        .def(
            ...
            [](const CppClass& self,
               ...
               const std::shared_ptr<BandwidthSelector> m_bselector,
               const std::string& variable,
               const std::vector<std::string>& evidence) { return self.vlocal_score_wbm(m, m_bselector, variable, evidence); },
            py::arg("model"),
            py::arg("m_bselector"),
            py::arg("variable"),
            py::arg("evidence"),
            R"doc(
vlocal_score(self: pybnesian.ValidatedScore, model: BayesianNetworkBase or ConditionalBayesianNetworkBase, m_bselector: BandwidthSelector, variable: str, evidence: List[str]) -> float

Returns the validated local score value of a node ``variable`` in the ``model`` if it had ``evidence`` as parents, with ``m_bselector`` as bandwidth selector.
.
...
:returns: Validated local score value of ``node`` in the ``model`` with ``evidence`` as parents.
        ...
        .def(
            ...
            [](CppClass& self,
               ...
               const std::string& variable,
               const std::vector<std::string>& evidence) {
                return self.vlocal_score_wbm(m, variable_type, m_bselector, variable, evidence);
            },
            py::arg("model"),
            py::arg("variable_type").none(false),
            py::arg("m_bselector"),
...
public:
    ...
    double local_score_wbm(const BayesianNetworkBase& model,
                       const std::shared_ptr<BandwidthSelector> m_bselector,
                       const std::string& variable,
                       const std::vector<std::string>& parents) const override {
        PYBIND11_OVERRIDE_PURE(double,      /* Return type */
                               ScoreBase,   /* Parent class */
                               local_score_wbm, /* Name of function in C++ (must match Python name) */
                               model.shared_from_this(),
                               m_bselector, /* Argument(s) */
                               variable, 
                               parents);
    }
...
public:
    ...
    double vlocal_score_wbm(const BayesianNetworkBase& model,
                        const std::shared_ptr<BandwidthSelector> m_bselector,
                        const std::string& variable,
                        const std::vector<std::string>& parents) const override {
        PYBIND11_OVERRIDE_PURE(double,             /* Return type */
                               ValidatedScoreBase, /* Parent class */
                               vlocal_score_wbm,       /* Name of function in C++ (must match Python name) */
                               model.shared_from_this(),
                               m_bselector, /* Argument(s) */
                               variable, 
                               parents);
