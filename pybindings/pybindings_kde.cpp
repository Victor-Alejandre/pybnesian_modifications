...
using kde::KDE, kde::ProductKDE, kde::BandwidthSelector, kde::ScottsBandwidth, kde::NormalReferenceRule, kde::UCV,
    kde::UCVScorer, kde::BCV, kde::BCVScorer, kde::SCV, kde::SCVScorer, kde::PI, kde::PIScorer;;
...
Initializes a :class:`UCV <pybnesian.UCV>`.
    ...
    py::class_<BCVScorer>(root, "BCVScorer")
        .def(py::init<const DataFrame&, const std::vector<std::string>&>())
        .def("score_diagonal", &BCVScorer::score_diagonal)
        .def("score_unconstrained", &BCVScorer::score_unconstrained);

    py::class_<BCV, BandwidthSelector, std::shared_ptr<BCV>>(root, "BCV", R"doc(
Selects the bandwidth using the Biased Cross Validation (BCV) criterion. It is only implemented for the full bandwidth matrix.

See Equation (3.9) in [MVKSA]_
)doc")
        .def(py::init<>(), R"doc(
Initializes a :class:`BCV <pybnesian.BCV>`.
)doc")
        .def(py::pickle([](const BCV& self) { return self.__getstate__(); },
                        [](py::tuple&) { return std::make_shared<BCV>(); }));  

    py::class_<SCVScorer>(root, "SCVScorer")
        .def(py::init<const DataFrame&, const std::vector<std::string>&>())
        .def("score_diagonal", &SCVScorer::score_diagonal)
        .def("score_unconstrained_mse", &SCVScorer::score_unconstrained_mse)
        .def("score_unconstrained", &SCVScorer::score_unconstrained);

    py::class_<SCV, BandwidthSelector, std::shared_ptr<SCV>>(root, "SCV", R"doc(
Selects the bandwidth using the Biased Cross Validation (SCV) criterion. It is only implemented for the full bandwidth matrix.

See Equation (3.9) in [MVKSA]_
)doc")
        .def(py::init<>(), R"doc(
Initializes a :class:`SCV <pybnesian.SCV>`.
)doc")
        .def(py::pickle([](const SCV& self) { return self.__getstate__(); },
                        [](py::tuple&) { return std::make_shared<SCV>(); })); 

        py::class_<PIScorer>(root, "PIScorer")
        .def(py::init<const DataFrame&, const std::vector<std::string>&>())
        .def("score_diagonal", &PIScorer::score_diagonal)
        .def("score_unconstrained_mse", &PIScorer::score_unconstrained_mse)
        .def("score_unconstrained", &PIScorer::score_unconstrained);

    py::class_<PI, BandwidthSelector, std::shared_ptr<PI>>(root, "PI", R"doc(
Selects the bandwidth using the Biased Cross Validation (PI) criterion. It is only implemented for the full bandwidth matrix.

)doc")
        .def(py::init<>(), R"doc(
Initializes a :class:`PI <pybnesian.PI>`.
)doc")
        .def(py::pickle([](const PI& self) { return self.__getstate__(); },
                        [](py::tuple&) { return std::make_shared<PI>(); })); 
