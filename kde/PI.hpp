#ifndef PYBNESIAN_KDE_PI_HPP
#define PYBNESIAN_KDE_PI_HPP
...
class PIScorer {
public:
    PIScorer(const DataFrame& df, const std::vector<std::string>& variables)
    ...
    double score_unconstrained(const MatrixXd& bandwidth, cl::Buffer psi_4) const;
...
private:
    ...
    double score_unconstrained_impl(
...
class PI : public BandwidthSelector {
public:
    ...
    std::string ToString() const override { return "Plug-in"; }

    py::tuple __getstate__() const override { return py::make_tuple(); }
    static std::shared_ptr<PI> __setstate__(py::tuple&) { return std::make_shared<PI>(); }
};

}  // namespace kde

#endif  // PYBNESIAN_KDE_PI_HPP